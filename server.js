...
function apiGet(urlString) {
  return new Promise((resolve, reject) => {
    https.get(
      urlString,
      { headers: { 'User-Agent': 'BetCore/2.0' } },
      r => {
        let data = '';
        ...
        r.on('end', () => {
          if (r.statusCode >= 200 && r.statusCode < 300) {
            try {
              ...
              reject(new Error('API повернув не JSON.'));
            }
          } else {
            reject(
              new Error(
...
function buildMatches(rows) {
  ...
  return [...map.values()].map(match => {
    ...
    for (const [marketKey, outcomes] of Object.entries(match.markets)) {
      ...
      // Best available price for every outcome/point.
...
function marketNoVig(outcomes) {
  const valid = (outcomes || []).filter(x => Number(x.price) > 1);
  if (valid.length < 2) return null;
  const sum = valid.reduce((acc, x) => acc + 1 / Number(x.price), 0);
  if (!(sum > 0)) return null;
  return valid.map(x => ({
    name: x.name,
    point: x.point ?? null,
    probability: (1 / Number(x.price)) / sum,
    fairOdds: sum > 0 ? sum / (1 / Number(x.price)) : null
  }));
}

function buildEngine(match) {
  // IMPORTANT: this is a market diagnostic, not an independent predictive model.
  // The BETCORE Fair Probability remains UNKNOWN until independent inputs
  // (team stats/xG/lineups/etc.) are integrated.
  ...
  if (h2h?.best?.length) {
    ...
    marketNoVig = marketNoVigFromBest(bestPrices);
  }

  const bookmakerCount = match.bookmakers?.length || 0;
  const marketCount = Object.keys(match.markets || {}).length;
  ...
  return {
    ...
    marketData: {
      bookmakerCount,
      marketCount,
      h2hAvailable: Boolean(h2h?.best?.length),
      oddsHistory: 'UNKNOWN',
      openingOdds: 'UNKNOWN',
    ...
    decision: 'SKIP',
    note: 'Ринковий no-vig — лише діагностика. BETCORE Edge не вигадується.'
  };
}

function marketNoVigFromBest(bestPrices) {
  const valid = bestPrices.filter(x => Number(x.price) > 1);
  if (valid.length < 2) return null;
  const overround = valid.reduce((acc, x) => acc + 1 / Number(x.price), 0);
...
function diagnostics(events, rows) {
  ...
  for (const event of Array.isArray(events) ? events : []) {
    for (const bookmaker of event.bookmakers || []) {
      bookmakerSet.add(bookmaker.key || bookmaker.title || 'unknown');
...
async function getOdds(req, res) {
  ...
  const sport = u.searchParams.get('sport') || 'soccer_epl';
  const regions = u.searchParams.get('regions') || 'eu';

  let markets =
    u.searchParams.get('markets') || 'h2h,totals,spreads';
  ...
  let result = await requestOdds(sport, regions, markets);
  let rows = normalize(result.data);
  let fallbackUsed = false;

  // If a combined football request returns nothing, retry the reliable 1X2 market.
  if (
    rows.length === 0 &&
    !requestedMarkets.includes('h2h') &&
    sport.startsWith('soccer_')
  ) {
    result = await requestOdds(sport, regions, 'h2h');
    rows = normalize(result.data);
    fallbackUsed = true;
    markets = 'h2h';
  }

  const diag = diagnostics(result.data, rows);
  const matches = buildMatches(rows).map(match => ({...match, engine: buildEngine(match)}));
  ...
  return send(
    ...
    JSON.stringify({
      ...
      quota: {
        ...
        last: result.headers['x-requests-last'] ?? null
      }
    }),
    {
...
async function handle(req, res) {
  ...
  if (req.url.startsWith('/api/odds')) {
    try {
      ...
      console.error('BetCore API error:', e);
      return send(
        res,
        502,
        'application/json; charset=utf-8',
        JSON.stringify({
          error: e.message || 'Не вдалося отримати букмекерську лінію.'
        })
      );
    }
  }
...
server.listen(PORT, () => {
  console.log(`BetCore 3.0 running on port ${PORT}`);
  console.log(`ODDS_API_KEY configured: ${Boolean(API_KEY)}`);
});
