...
function analyzeMovement(match, historyRows) {
  ...
  for (const cb of current) {
    ...
    for (const co of cb.outcomes || []) {
      ...
      if (
        !po ||
        !Number.isFinite(Number(co.price)) ||
        !Number.isFinite(Number(po.price))
      ) continue;
  ...
  const outcomes = [...outcomeMap.values()].map(item => {
    ...
    const consensus =
      total === 0 ? 'FLAT' :
      item.down > item.up ? 'DOWN' :
      item.up > item.down ? 'UP' :
      'MIXED';
  ...
  const changedBookmakers = new Set(
    details.map(x => x.bookmakerKey || x.bookmaker)
  ).size;
  ...
  const downBooks = new Set(
    details.filter(x => x.direction === 'DOWN').map(x => x.bookmakerKey || x.bookmaker)
  ).size;
  const upBooks = new Set(
    details.filter(x => x.direction === 'UP').map(x => x.bookmakerKey || x.bookmaker)
  ).size;
  ...
  return {
    ...
    bookmakersCompared: new Set(
      current.map(x => x.bookmakerKey || x.bookmaker)
    ).size,
    changedBookmakers,
    up,
    down,
...
http.createServer(handle).listen(
  PORT,
  () => console.log(`BetCore 4.9 running on ${PORT}`)
);
