...
function apiGet(urlString) {
  return new Promise((resolve, reject) => {
    const req = https.get(urlString, { headers: { 'User-Agent': 'BetCore/4.6' } }, r => {
      let data = '';
      r.on('data', c => data += c);
      r.on('end', () => {
...
function analyzeMovement(match, historyRows) {
  ...
  if (!current.length || !Array.isArray(historyRows) || historyRows.length < 1) {
    return { status:'INSUFFICIENT_HISTORY', samples:historyRows?.length||0, bookmakersCompared:0, changedBookmakers:0, up:0, down:0, flat:0, consensus:'UNKNOWN', syncCount:0, syncDirection:'UNKNOWN', details:[] };
  }

  const previous = historyRows[historyRows.length - 1]?.bookmakers || [];
  const details = [];
  let up=0, down=0, flat=0;

  for (const cb of current) {
    const pb = previous.find(x => x.bookmakerKey === cb.bookmakerKey || x.bookmaker === cb.bookmaker);
    if (!pb) continue;
    for (const co of cb.outcomes || []) {
      ...
      const from=Number(po.price), to=Number(co.price), delta=Number((to-from).toFixed(3));
      if (Math.abs(delta)<0.005) { flat++; continue; }
      const direction=delta<0?'DOWN':'UP';
      if(direction==='DOWN') down++; else up++;
      details.push({bookmaker:cb.bookmaker,bookmakerKey:cb.bookmakerKey,outcome:co.name,from,to,delta,direction});
    }
  }
  ...
  const syncDirection=downBooks===upBooks?(syncCount?'MIXED':'UNKNOWN'):downBooks>upBooks?'DOWN':'UP';

  return {
    status:details.length?'AVAILABLE':'FLAT', samples:historyRows.length,
    bookmakersCompared:new Set(current.map(x=>x.bookmakerKey||x.bookmaker)).size,
    changedBookmakers, up, down, flat, consensus, syncCount, syncDirection,
...
function buildEngine(match, historyRows) {
  ...
  const movement = analyzeMovement(match, hist);
...
http.createServer(handle).listen(
  PORT,
  () => console.log(`BetCore 4.6 running on ${PORT}`)
);
