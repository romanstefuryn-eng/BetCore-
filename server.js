...
function apiGet(urlString) {
  return new Promise((resolve, reject) => {
    const req = https.get(urlString, { headers: { 'User-Agent': 'BetCore/4.3' } }, r => {
      let data = '';
      r.on('data', c => data += c);
      r.on('end', () => {
...
function isSoccer(s) {
  ...
  return (key.startsWith('soccer_') || group.includes('soccer')) && !s.has_outrights;
}
...
async function getOdds(req, res) {
  ...
  try {
    ...
    return send(
      ...
      502,
      'application/json; charset=utf-8',
      JSON.stringify({ error: e.message, stage: 'sports' })
    );
  }
  ...
  let targets =
    scope === 'sport' && sport
      ? allSoccer.filter(s => s.sport_key === sport)
      : allSoccer;
  ...
  for (const event of futureEvents) {
    if (!bySport.has(event.sport_key)) {
      bySport.set(event.sport_key, []);
    }

    bySport.get(event.sport_key).push(event);
  }

  let activeTargets = targets.filter(
    s => bySport.has(s.sport_key)
  );

  activeTargets.sort((a, b) => {
    const ca = bySport.get(a.sport_key).length;
    const cb = bySport.get(b.sport_key).length;
  ...
  /*
    FALLBACK:
    If /events returned no usable events,
    query /odds directly.
  */
  ...
  if (futureEvents.length === 0) {
    ...
    const fallbackTargets =
      targets.slice(0, MAX_ODDS_SPORTS);
    ...
    for (
      ...
      for (const x of results) {
        ...
        remaining =
          x.r.headers['x-requests-remaining'] ??
          remaining;

        used =
          x.r.headers['x-requests-used'] ??
          used;
  ...
  for (
    let i = 0;
    i < oddsTargets.length;
    i += oddsConcurrency
  ) {
    const batch = oddsTargets.slice(
      i,
      i + oddsConcurrency
    );
  ...
  const sportsReturned =
    new Set(
      rows.map(x => x.sportKey)
    ).size;
  ...
  try {
    ...
    if (u.pathname === '/api/debug') {
      ...
      try {
        ...
        const soccer = raw.filter(isSoccer);
        ...
        return send(res, 200, 'application/json; charset=utf-8', JSON.stringify({
          ...
          firstSports: raw.slice(0, 20).map(s => ({
            key: s.sport_key,
            title: s.title,
            active: s.active,
