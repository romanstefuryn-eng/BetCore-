...
async function getOdds(req, res) {
  ...
  const payload = {
    ...
    coverage: {
      ...
      matchedFixtureOdds: matches.filter(m => m.fixtureSource === 'SofaScore + The Odds API').length,
    ...
    diagnostics: {
      version: '5.3',
      eventsMethod: 'SofaScore fixture universe (inverse + normal, dual host) + The Odds API market merge',
      fallbackUsed,
      fixtureSource: fixtureResult.source,
      fixtureErrors: fixtureResult.errors || [],
      coverageSummary: {
        fixtureRaw: fixtureResult.events?.length || 0,
        fixtureAllowed: sofaFixtures.length,
        futureUniverse: futureMerged.length,
        returned: matches.length,
        withOdds,
        withoutOdds: matches.length - withOdds,
        matchedAcrossSources: matches.filter(m => m.fixtureSource === 'SofaScore + The Odds API').length,
        cappedByMaxMatches: capped
      },
...
http.createServer(handle).listen(
  PORT,
  () => console.log(`BetCore 5.3 running on ${PORT}`)
);
