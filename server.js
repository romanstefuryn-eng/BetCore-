      ...
      r.on('end', () => {
        if (r.statusCode >= 200 && r.statusCode < 300) {
          try {
            ...
            reject(new Error('API-Football повернув не JSON.'));
          }
        } else {
          reject(new Error(`API-Football HTTP ${r.statusCode}: ${data.slice(0, 500)}`));
        }
      });
    });
    req.on('error', reject);
...
function mergeOddsIntoFixtures(fixtures, oddsMatches) {
  ...
  for (const f of fixtures) {
    ...
    if (closeEnough) {
      ...
      merged.push({ ...f, ...candidate, fixtureSource: `${f.fixtureSource || 'Fixture source'} + The Odds API` });
    } else {
      merged.push(f);
    }
  }
...
async function getOdds(req, res) {
      ...
      if (fixtureResult.source === 'api-football' && e.fixture) {
        return {
          ...
          fixtureMeta: {
            ...
            season: e.league?.season || null,
            round: e.league?.round || null,
  ...
  const payload = {
    ...
    coverage: {
      ...
      matchedFixtureOdds: matches.filter(m => String(m.fixtureSource || '').includes(' + The Odds API')).length,
    ...
    diagnostics: {
      version: '5.4',
      eventsMethod: fixtureResult.source === 'api-football'
        ? 'API-Football global fixture universe + The Odds API market merge'
        : 'SofaScore fixture universe (inverse + normal, dual host) + The Odds API market merge',
      fallbackUsed,
      fixtureSource: fixtureResult.source,
      ...
      coverageSummary: {
        ...
        matchedAcrossSources: matches.filter(m => String(m.fixtureSource || '').includes(' + The Odds API')).length,
        cappedByMaxMatches: capped
      },
...
http.createServer(handle).listen(
  PORT,
  () => console.log(`BetCore 5.4 running on ${PORT}`)
);
